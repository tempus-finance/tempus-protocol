...
contract TempusIncentivization is StakingRewards, ReentrancyGuard, Ownable, Versioned {
    ...
    constructor(address rewardsToken, address stakingToken)
        StakingRewards(rewardsToken, stakingToken)
        Versioned(1, 0, 0)
    {}
    ...
    function depositAndStakeYields(
        ...
        ITempusController controller = ITempusController(tempusPool.controller());

        IERC20 capitals = IERC20(address(tempusPool.principalShare()));
        IERC20 yields = IERC20(address(tempusPool.yieldShare()));

        uint256 mintedShares;
        if (isBackingToken) {
            IERC20(tempusPool.backingToken()).safeTransferFrom(msg.sender, address(this), tokenAmount);
            mintedShares = controller.depositBacking(tempusPool, tokenAmount, address(this));
        }
        else {
            IERC20(tempusPool.yieldBearingToken()).safeTransferFrom(msg.sender, address(this), tokenAmount);
            mintedShares = controller.depositYieldBearing(tempusPool, tokenAmount, address(this));
        }
        ...
        uint256 minReturn = swapAmount.divfV(yieldsRate, tempusPool.backingTokenONE());
        ...
        _stake(yieldsBalance - yieldsBalancePreSwap, msg.sender);
    ...
    function unstakeYieldsAndExitAmmGivenLpAndRedeem(
        ...
        {
            IERC20 capitals = IERC20(address(controllerPayload.tempusPool.principalShare()));
            IERC20 yields = IERC20(address(controllerPayload.tempusPool.yieldShare()));

            if (controllerPayload.lpTokens > 0) {
                require(
                    controllerPayload.tempusAMM.transferFrom(msg.sender, address(this), controllerPayload.lpTokens),
        ...
        uint256 yieldsUnstaked = _exit(msg.sender);
        ...
        ITempusController(controllerPayload.tempusPool.controller()).exitAmmGivenLpAndRedeem(
            controllerPayload.tempusAMM,
            controllerPayload.tempusPool,
            controllerPayload.lpTokens,
            controllerPayload.principals,
            controllerPayload.yields + yieldsUnstaked,
